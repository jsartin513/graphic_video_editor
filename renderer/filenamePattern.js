/** Browser copy of src/filename-pattern.js (renderer ES modules). */

export const GENERIC_FILENAME_PATTERN = '{date} {sessionId}';

const PROCESSED_PLACEHOLDER_RE = /^PROCESSED\d{4}/i;

export function chooseDefaultFilenamePattern(preferences) {
...
export function isProcessedPlaceholderFilename(name) {
  if (!name || typeof name !== 'string') return false;
  const base = name.replace(/\.(mp4|mov|mkv|avi|m4v)$/i, '').trim();
  return PROCESSED_PLACEHOLDER_RE.test(base);
}

export function directorySuffixForGroup(group, allGroups) {
